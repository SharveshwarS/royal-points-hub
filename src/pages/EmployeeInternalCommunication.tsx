...
const EmployeeInternalCommunication = () => {
  ...
  useEffect(() => {
    const fetchAdmins = async () => {
      try {
        ...
        const admins = data.filter((u: User) => u.role?.toLowerCase() === "admin");
        setEmployees(admins);
      } catch (error) {
    ...
    const fetchMessages = async () => {
      try {
        const res = await fetch(
          `https://royal-points-hub-production.up.railway.app/api/chat/conversation?user1=${employeeId}&user2=${selectedUser.id}`
        );
        if (!res.ok) return;
        const data = await res.json();
        setMessages(data);
...
// 🛑 2. Reset the scroll lock whenever you click a different Admin
  // useEffect(() => {
...
useEffect(() => {
  ...
  // 🛑 3. Replace your old auto-scroll useEffect with this one
useEffect(() => {
...
const sendMessage = async () => {
    ...
    try {
      ...
      // 🛑 4. Force a smooth scroll down ONLY when you manually send a message
      setTimeout(() => {
        messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
      }, 50);
  ...
  if (!user) return <div className="p-4 text-gray-500">Loading user profile...</div>;

  return (
    // 🛑 ADDED: min-h-0 and flex-1 to the absolute root of this component
    <div className="flex flex-1 w-full h-full min-h-0 overflow-hidden bg-white text-gray-800 font-sans">
      ...
      <div className="w-[30%] flex flex-col h-full border-r border-gray-200 bg-white shrink-0">
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {employees.length === 0 ? (
            <p className="text-gray-500 p-4 text-center mt-4">No admins found</p>
          ) : (
            employees.map(emp => (
              <div
                key={emp.id}
      ...
      {/* 🛑 ADDED: flex-1 and min-h-0 here as well */}
...
export default EmployeeInternalCommunication;